...
git clone https://github.com/shreyashagrawal00/FakeShield.git
cd FakeShield2.0

# Make sure your trained model files are in place
# backend/models/image_model.h5
...
> **Note:** The model weight files (`*.h5`) are mounted from `backend/models/` via a Docker volume — they are **not** baked into the image. Make sure they exist before starting.
...
```bash
docker pull shreyashagrawal00/fakeshield-backend:latest
docker pull shreyashagrawal00/fakeshield-frontend:latest
